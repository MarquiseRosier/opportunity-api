...
async function openPage(url, browser) {
...
async function getBoundingBoxes(page, source, target, url) {
  const { sourceBoxes, targetBoxes } = await page.evaluate(({ source, target, url }) => {
...
async function removeZeros(obj) {
  const res = [];
...
function findIntersections(sources = [], targets = []) {
  const intersects = [];
